#!/bin/bash
# ==========================================
#  LevelUp - 智能工作 Harness (V3)
# ==========================================
# 用途：每天早上 8 点自动启动，下午 2 点自动汇报
# 测试命令：bash smart-harness-v3.sh --test
...
while true; do
    ...
    if [ "$current_min" == "00" ]; then
        ...
        if npm run build > /tmp/build.log 2>&1; then
            ...
            echo_log "  ❌ 构建失败"
            tail -3 /tmp/build.log | sed 's/^/     /' | tee -a "$LOG_FILE"
        fi
    fi
    
    # 3. 检查 TypeScript（只在半点检查）
    if [ "$current_min" == "30" ]; then
        echo_log "  [3/3] 检查 TypeScript..."
